...
class ClaudeCodeAdapter(UniversalAgentInterface):
    ...
    def _translate_task_to_command(self, task: AgentTask) -> ClaudeCodeCommand:
        ...
        Maps task types to appropriate Claude Code commands and formats arguments.
    ...
    async def _execute_claude_command(
        self, command: ClaudeCodeCommand, context: Optional[ExecutionContext]
    ) -> ClaudeCodeResponse:
        """
        ...
        try:
            ...
            if success and stdout_str:
                try:
                    ...
                    output = {"raw_output": stdout_str, "command": command.command, "success": True}
            elif not success:
                output = {
                    "error": stderr_str,
    ...
    def _validate_command_safety(self, command: ClaudeCodeCommand) -> None:
        ...
        try:
            ...
            if task.type not in supported_types:
                raise TaskExecutionError(
                    f"Task type {task.type} not supported by Claude Code adapter"
                )
            ...
            if task.timeout_seconds > self._resource_limits.get("max_execution_time", 3600):
                raise TaskExecutionError(
                    f"Task timeout {task.timeout_seconds}s exceeds maximum allowed"
                )
    ...
    def _is_safe_env_var(self, key: str, value: str) -> bool:
        ...
        Prevents environment variable injection and privilege escalation attempts.
        ...
        Returns:
            True if environment variable is safe
        """
        # Dangerous environment variables that could lead to privilege escalation
        unsafe_keys = {
            "PATH",
            "LD_LIBRARY_PATH",
            "PYTHONPATH",
        ...
        # Block any sudo-related variables
        sudo_prefixes = ("SUDO_", "SUDOERS_")

        # Security checks
        if key.upper() in unsafe_keys:
            return False

        if any(key.upper().startswith(prefix) for prefix in sudo_prefixes):
            return False

        # Block shell injection attempts in values
        dangerous_patterns = [
            ";",
            "&&",
            "||",
    ...
    async def get_capabilities(self) -> List[AgentCapability]:
        """
        Report Claude Code capabilities with dynamic assessment.

        Returns accurate capability assessments based on Claude Code's actual
        ...
        try:
            ...
            if (
                ...
                and (current_time - self._last_health_check).total_seconds() < 300
            ):
            ...
            # 4. Define Claude Code's core capabilities with dynamic adjustments
            capabilities = []
    ...
    def _calculate_load_factor(self, cpu_usage: float, memory_usage: float) -> float:
        ...
        active_task_factor = max(
            0.3, 1.0 - (len(self._current_tasks) / self._max_concurrent_tasks) * 0.5
        )
    ...
    async def health_check(self) -> HealthStatus:
        """
        Perform comprehensive health check of Claude Code adapter.
        ...
        try:
            logger.debug("Starting health check for Claude Code adapter")
            ...
            critical_issues = []
            if not cli_available:
                critical_issues.append("Claude Code CLI unavailable")
            ...
            if capacity_utilization > 0.9:
                critical_issues.append(
                    f"Near capacity: {current_load}/{self._max_concurrent_tasks}"
                )
    ...
    async def _check_working_directory(self) -> bool:
        ...
        try:
            ...
            test_file = os.path.join(
                self._working_directory, f".health_test_{uuid.uuid4().hex[:8]}"
            )
            try:
        ...
        try:
            ...
            try:
                with open("/proc/loadavg", "r") as f:
                    ...
                    # This is an approximation: load_avg * 100 / cpu_count
                    try:
                        import os
    ...
    def _determine_health_state(
        ...
        Args:
            cli_available: Whether Claude Code CLI is available
...
def create_claude_code_adapter(
    agent_id: str, cli_path: str = "claude", working_directory: str = "/tmp"
) -> ClaudeCodeAdapter:
    """
